...
const singleOperationReportTemplate =`
---
title: ${reportName}
date: ${today}
---
...
Unsuccessful API Calls : ${namesOfFilesReceivingErrors.length - namesOfFilesCorrected.length}
...
function writeOutput(markdown, options) {
  ...
  const existingFiles = fs.readdirSync(dir).filter(file => file.match(new RegExp(pattern)));
