...
const CONTENT_DIR = path.resolve(process.cwd(), USER_OPTIONS.directories.content || 'site/src/content');
...
// Properties that should never contain quotes (URLs, etc.)
 ...
 */
function extractPlainTextFrontmatter(content) {
...
/**
 * Screens a file to check if it has valid frontmatter
 * @param {string} content - Raw file content
 * @param {string} filePath - Path to the file
 * @param {string} fileName - Name of the file (without extension)
